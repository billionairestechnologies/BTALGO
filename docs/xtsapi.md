# ⚙️ How to Integrate Any XTS API-Supported Broker in BTAlgo (5-Minute Setup)

BTAlgo already supports XTS API through the `compositedge` plugin. Any broker using XTS (like IIFL, Nirmal Bang, Anand Rathi, Jainam, 5paisa, etc.) can be added with **zero code duplication**—just a few config updates.
...
│
└── mapping/
    ├── order_data.py           # BTAlgo → XTS order translation
    └── transform_data.py       # XTS → BTAlgo data formatting
```
...
```json
{
  ...
  "Description": "CompositedgeBTAlgo Plugin",
  "Version": "1.0",
  "Author": "Kalaivani",
...
Thanks to BTAlgo’s modular and broker-agnostic design:
