...
BTAlgo runs on Ubuntu, Debian, Raspbian, RHEL, Rocky, AlmaLinux, Amazon Linux, Fedora, and Arch — the installation script auto-detects the distro and uses the right package manager. The flow below uses Ubuntu as the example.
...
* Obtain your broker's API credentials per the BTAlgo documentation:
  * API Key
  * API Secret
...
* Set up the BTAlgo application under `/var/python/btalgo`
* Create the systemd unit `btalgo.service`
* Generate timestamped installation logs in `~/btalgo-install/logs/`
...
```
/var/python/btalgo/                  cloned repo
/var/python/btalgo/.venv/            uv-managed Python virtual environment
/var/python/btalgo/.env              configuration
/var/python/btalgo/btalgo.sock     Gunicorn Unix socket
/etc/systemd/system/btalgo.service   systemd unit
/etc/nginx/sites-available/btalgo.conf   Nginx vhost (stable name across domain changes)
```

The Nginx vhost name `btalgo.conf` is intentionally fixed — `install/change-domain.sh` updates `server_name` in place rather than renaming the file.

#### Multi-Domain Deployment (Optional)

The default `install.sh` is single-deploy per server. If you need 2+ BTAlgo instances side by side (different broker per instance, etc.), use the dedicated multi-deploy installer:

```bash
wget https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/install/install-multi.sh
...
```
/var/python/btalgo-flask/<domain-broker>/btalgo/
/var/python/btalgo-flask/<domain-broker>/venv/
btalgo-<domain-broker>.service
```

Each deployment gets its own service, configuration, virtual environment, SSL certificate, and log file. The single-deploy `update.sh` and `change-domain.sh` scripts also handle this layout transparently — they detect the simple path first and fall back to scanning `/var/python/btalgo-flask/`.
...
2. **Test the Nginx configuration**
   ...
   ```bash
   sudo nginx -t
   ls -l /etc/nginx/sites-enabled/btalgo.conf
   ```
3. **Open the dashboard** at `https://yourdomain.com`
4. **View installation log**

   ```bash
   ls -l ~/btalgo-install/logs/
   cat ~/btalgo-install/logs/install_YYYYMMDD_HHMMSS.log
   ```

### Remote MCP

Remote MCP exposes `/mcp` and `/oauth/*` so hosted AI clients (claude.ai, chatgpt.com) can connect to your BTAlgo install over HTTPS. Local stdio MCP (Claude Desktop, Cursor, Windsurf) is unaffected — it works regardless of this setting.
...
2. **Application not starting**
   ...
   ```bash
   # View live logs
   sudo journalctl -fu btalgo

   # Last 100 lines
   sudo journalctl -n 100 -u btalgo
...
4. **Installation logs**
   ...
   ```bash
   ls -l ~/btalgo-install/logs/
   cat ~/btalgo-install/logs/$(ls -t ~/btalgo-install/logs/ | head -1)
   ```
...
6. **Inspect the install directly**
   ...
   ```bash
   # Repo
   ls /var/python/btalgo

   # Effective configuration
   sudo cat /var/python/btalgo/.env

   # Application logs
   sudo tail -f /var/python/btalgo/log/btalgo_$(date +%F).log

   # JSON-formatted error log (always-on)
   sudo tail -f /var/python/btalgo/log/errors.jsonl
   ```
...
```bash
# List all BTAlgo services on this host
systemctl list-units 'btalgo*'

# Manage a specific deployment (example: trading1.yourdomain.com + Fyers)
sudo systemctl status btalgo-trading1-yourdomain-com-fyers
sudo journalctl -fu btalgo-trading1-yourdomain-com-fyers

# Per-deployment install directories
ls /var/python/btalgo-flask/
```
...
```bash
cd ~/btalgo-install
wget https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/install/update.sh
chmod +x update.sh
sudo ./update.sh
```

The update script detects both layouts (single-deploy at `/var/python/btalgo` and legacy multi-deploy under `/var/python/btalgo-flask/`) and asks which to update if multiple are present. It runs `git pull`, `uv sync`, and restarts the service.

### Changing the Domain

```bash
cd ~/btalgo-install
wget https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/install/change-domain.sh
...
5. **Single user per deployment**
   ...
   BTAlgo is designed for one trader per server. There is no multi-user model — server access equals full control of the broker session. Don't share the host.
...
* GitHub: [github.com/billionairestechnologies/btalgo](https://github.com/billionairestechnologies/btalgo)
* Documentation: [docs.billionairestechnologies.com](https://docs.billionairestechnologies.com)
* Discord: [billionairestechnologies.com/discord](https://www.billionairestechnologies.com/discord)
...
```
GET https://docs.billionairestechnologies.com/installation-guidelines/getting-started/ubuntu-server-installation.md?ask=<question>
```
