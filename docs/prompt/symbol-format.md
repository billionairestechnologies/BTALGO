...
3. **NSE Equity for State Bank of India:** If the base symbol is `SBIN`, the BTAlgo symbol on NSE would be `SBIN`.

### Future Symbol Format

For futures, the BTAlgo symbology specifies that the symbol should consist of the base symbol followed by the expiration date and "FUT" to denote that it is a futures contract.
...
* **Example:** For government bond futures, specifically the 7.26% 2033 bond expiring in April 2024, the symbol in BTAlgo would be `726GS203325APR24FUT`.

### Options Symbol Format

Options symbols in BTAlgo are structured to include the base symbol, the expiration date, the strike price, and whether it's a Call or Put option.
...
The supported exchange symbol formats in BTAlgo allow for an identification system that denotes where the instrument is traded, along with specific details that vary by instrument type:
...
For developers, understanding the database schema is essential for managing data effectively within BTAlgo:

1. **id:** A unique identifier for each record in the database.
2. **symbol:** The standard trading symbol of the instrument as per BTAlgo's symbology.
...
5. **exchange:** The standard exchange identifier code (e.g NSE, BSE, MCX CDS etc) where the instrument is traded as per BTAlgo's symbology.
...
This schema captures both the standardized BTAlgo symbology and the potentially divergent broker-specific information, enabling algorithms and traders to operate across multiple platforms without confusion. It allows for the storage of instrument metadata necessary for trading activities and ensures that all financial instruments are identifiable and their market details readily accessible.
...
```
GET https://docs.billionairestechnologies.com/symbol-format.md?ask=<question>
```
