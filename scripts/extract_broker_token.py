"""
Extract the broker access token from this BTAlgo deployment's own database.

Intended for the OWNER of a self-hosted BTAlgo instance who wants to call
broker-native endpoints BTAlgo does not proxy (e.g., Upstox option-chain
Greeks). This script reuses BTAlgo's existing Fernet decryption — it does
NOT re-implement crypto.

PRECONDITIONS:
  - Run inside the BTAlgo venv:    uv run python scripts/extract_broker_token.py
  - .env must contain the same API_KEY_PEPPER used to encrypt the row
  - db/btalgo.db must contain an active Auth row (i.e. you've logged in today
    via the BTAlgo UI — Indian broker tokens expire daily ~3:00 AM IST)
...
SECURITY NOTES (read before using):
  ...
  - Calls you make directly against the broker bypass BTAlgo's traffic logs,
  ...
  - Prefer building the missing feature inside BTAlgo over scripting against
    a leaked token.
...
# Make sure we can import BTAlgo's modules from repo root.
...
def _banner(stream) -> None:
    ...
    stream.write("  - Direct broker calls bypass BTAlgo's audit & rate limits\n")
    stream.write("=" * 72 + "\n")
...
def main(as_json: bool = False) -> int:
    ...
    if not rows:
        sys.stderr.write(
            "No auth rows in db/btalgo.db. Log in via the BTAlgo UI to "
