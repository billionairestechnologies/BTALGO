...
# BTAlgo Docker Multi-Instance Installation with Custom SSL
...
DEFAULT_REPO="https://github.com/billionairestechnologies/btalgo.git"
...
for i in "${!CONF_DOMAINS[@]}"; do
    ...
    PROJECT_NAME="btalgo-${SANITIZED_NAME}"
    ...
    if [ "$IS_UPDATE_MODE" == "true" ] && [ -f "$ENV_FILE" ]; then
        ...
        sed -i "s|BTALGO_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" "$ENV_FILE"
        sed -i "s|BTALGO_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$PEPPER|g" "$ENV_FILE"
        ...
        echo "BTALGO_GIT_BRANCH = '${GIT_BRANCH}'" >> "$ENV_FILE"
        echo "BTALGO_GIT_COMMIT = '${GIT_COMMIT}'" >> "$ENV_FILE"
...
upstream btalgo_flask_${SANITIZED_NAME} {
    server 127.0.0.1:${FLASK_PORT};
    keepalive 64;
}

upstream btalgo_websocket_${SANITIZED_NAME} {
...
server {
    ...
    # Logic: WebSocket
    location = /ws {
        proxy_pass http://btalgo_websocket_${SANITIZED_NAME};
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
    ...
    location /socket.io/ {
        proxy_pass http://btalgo_flask_${SANITIZED_NAME}/socket.io/;
        proxy_http_version 1.1;
        proxy_read_timeout 86400s;
    ...
    # Logic: Main App
    location / {
        proxy_pass http://btalgo_flask_${SANITIZED_NAME};
        proxy_http_version 1.1;
...
cat <<'EOF' > /usr/local/bin/btalgo-ctl
#!/bin/bash
# BTAlgo Manager
...
usage() {
    echo "Usage: btalgo-ctl <command> [domain]"
    echo "Commands:"
...
chmod +x /usr/local/bin/btalgo-ctl


log "\n==============================================" "$GREEN"
log " INSTALLATION COMPLETE" "$GREEN"
log "==============================================" "$GREEN"
log "Management Command: btalgo-ctl" "$BLUE"
log "  btalgo-ctl list" "$BLUE"
log "  btalgo-ctl restart <domain.com>" "$BLUE"
log "  btalgo-ctl logs <domain.com>" "$BLUE"
