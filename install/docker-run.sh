#!/bin/bash
# ============================================================================
# BTAlgo Docker Runner for macOS/Linux
# ============================================================================
#
# Quick Start (2 commands):
#   1. Download: curl -O https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/install/docker-run.sh && chmod +x docker-run.sh
#   2. Run:      ./docker-run.sh
#
# Commands:
#   start    - Start BTAlgo container (default, runs setup if needed)
...
# Configuration
IMAGE="marketcalls/btalgo:latest"
CONTAINER="btalgo"
ENV_FILE=".env"
SAMPLE_ENV_URL="https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/.sample.env"
# Use the directory where the script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BTALGO_DIR="$SCRIPT_DIR"
...
echo ""
echo -e "${BLUE}  ========================================${NC}"
echo -e "${BLUE}       BTAlgo Docker Runner${NC}"
...
# Setup function
do_setup() {
    log_info "Setting up BTAlgo in $BTALGO_DIR..."
    echo ""

    # Create db directory
    if [ ! -d "$BTALGO_DIR/db" ]; then
        log_info "Creating database directory..."
        mkdir -p "$BTALGO_DIR/db"
        if [ $? -ne 0 ]; then
    ...
    if [ -f "$BTALGO_DIR/$ENV_FILE" ]; then
        log_warn ".env file already exists at $BTALGO_DIR/$ENV_FILE"
    ...
    if ! curl -sL "$SAMPLE_ENV_URL" -o "$BTALGO_DIR/$ENV_FILE"; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS sed syntax
        sed -i '' "s/BTALGO_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE/$APP_KEY/g" "$BTALGO_DIR/$ENV_FILE"
        sed -i '' "s/BTALGO_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE/$API_KEY_PEPPER/g" "$BTALGO_DIR/$ENV_FILE"
    else
        # Linux sed syntax
        sed -i "s/BTALGO_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE/$APP_KEY/g" "$BTALGO_DIR/$ENV_FILE"
        sed -i "s/BTALGO_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE/$API_KEY_PEPPER/g" "$BTALGO_DIR/$ENV_FILE"
    ...
    if [ "$(uname)" = "Linux" ]; then
        if [ "$EUID" -ne 0 ] && command -v sudo >/dev/null 2>&1; then
            sudo chown 1000:1000 "$BTALGO_DIR/$ENV_FILE" 2>/dev/null || true
            sudo chmod 600 "$BTALGO_DIR/$ENV_FILE" 2>/dev/null || true
        else
            chown 1000:1000 "$BTALGO_DIR/$ENV_FILE" 2>/dev/null || true
            chmod 600 "$BTALGO_DIR/$ENV_FILE" 2>/dev/null || true
        fi
    else
        chmod 600 "$BTALGO_DIR/$ENV_FILE" 2>/dev/null || true
    fi
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS sed syntax
        sed -i '' "s/BROKER_API_KEY = 'YOUR_BROKER_API_KEY'/BROKER_API_KEY = '$BROKER_API_KEY'/g" "$BTALGO_DIR/$ENV_FILE"
        sed -i '' "s/BROKER_API_SECRET = 'YOUR_BROKER_API_SECRET'/BROKER_API_SECRET = '$BROKER_API_SECRET'/g" "$BTALGO_DIR/$ENV_FILE"
        sed -i '' "s|<broker>|$BROKER_NAME|g" "$BTALGO_DIR/$ENV_FILE"

        if [ "$IS_XTS" -eq 1 ]; then
            sed -i '' "s/BROKER_API_KEY_MARKET = 'YOUR_BROKER_MARKET_API_KEY'/BROKER_API_KEY_MARKET = '$BROKER_API_KEY_MARKET'/g" "$BTALGO_DIR/$ENV_FILE"
            sed -i '' "s/BROKER_API_SECRET_MARKET = 'YOUR_BROKER_MARKET_API_SECRET'/BROKER_API_SECRET_MARKET = '$BROKER_API_SECRET_MARKET'/g" "$BTALGO_DIR/$ENV_FILE"
        fi
    else
        # Linux sed syntax
        sed -i "s/BROKER_API_KEY = 'YOUR_BROKER_API_KEY'/BROKER_API_KEY = '$BROKER_API_KEY'/g" "$BTALGO_DIR/$ENV_FILE"
    ...
    if [[ "$OPEN_EDITOR" =~ ^[Yy]$ ]]; then
        if [[ "$OSTYPE" == "darwin"* ]]; then
            open -t "$BTALGO_DIR/$ENV_FILE"
        elif command -v xdg-open &> /dev/null; then
            # Linux with desktop environment - non-blocking
            xdg-open "$BTALGO_DIR/$ENV_FILE" &>/dev/null &
        elif command -v gedit &> /dev/null; then
            gedit "$BTALGO_DIR/$ENV_FILE" &>/dev/null &
        elif command -v code &> /dev/null; then
            code "$BTALGO_DIR/$ENV_FILE"
        else
            echo "  Edit .env manually: $BTALGO_DIR/$ENV_FILE"
        fi
    fi

    echo ""
    log_ok "Setup complete! Run './docker-run.sh start' to launch BTAlgo."
    echo ""
    return 0
}
...
do_start() {
    ...
    if [ ! -f "$BTALGO_DIR/$ENV_FILE" ]; then
        ...
        if ! do_setup; then
            echo ""
            log_error "Setup failed. Cannot start BTAlgo."
            echo "Please fix the issues above and try again."
            exit 1
        fi
        echo ""
        log_info "Starting BTAlgo after setup..."
        echo ""
    fi
    ...
    if [ ! -d "$BTALGO_DIR/strategies" ]; then
        log_info "Creating strategies directory..."
        mkdir -p "$BTALGO_DIR/strategies/scripts"
        mkdir -p "$BTALGO_DIR/strategies/examples"
    fi
    if [ ! -d "$BTALGO_DIR/log" ]; then
        log_info "Creating log directory..."
        mkdir -p "$BTALGO_DIR/log/strategies"
    fi
    if [ ! -d "$BTALGO_DIR/keys" ]; then
        log_info "Creating keys directory..."
        mkdir -p "$BTALGO_DIR/keys"
    fi
    if [ ! -d "$BTALGO_DIR/tmp" ]; then
        log_info "Creating temp directory..."
        mkdir -p "$BTALGO_DIR/tmp"
    fi
    ...
    if docker run -d \
        ...
        -v "$BTALGO_DIR/tmp:/app/tmp" \
        -v "$BTALGO_DIR/.env:/app/.env" \
        --restart unless-stopped \
        "$IMAGE"; then

        echo ""
        log_success "BTAlgo started successfully!"
        echo ""
        echo -e "${GREEN}  ========================================${NC}"
        ...
        echo "    ./docker-run.sh stop     - Stop BTAlgo"
        echo "    ./docker-run.sh restart  - Restart BTAlgo"
        echo ""
    else
        echo ""
...
# Stop function
do_stop() {
    log_info "Stopping BTAlgo..."
    docker stop "$CONTAINER" >/dev/null 2>&1
    docker rm "$CONTAINER" >/dev/null 2>&1
    log_ok "BTAlgo stopped."
}

# Restart function
do_restart() {
    log_info "Restarting BTAlgo..."
    do_stop
    echo ""
    do_start
}
...
do_status() {
    ...
    if docker ps --filter "name=$CONTAINER" --filter "status=running" | grep -q "$CONTAINER"; then
        echo -e "${GREEN}[STATUS]${NC} BTAlgo is running."
        echo ""
        echo "  Web UI: http://127.0.0.1:5000"
    else
        echo -e "${YELLOW}[STATUS]${NC} BTAlgo is NOT running."
    fi
    echo ""
    echo "  Data directory: $BTALGO_DIR"
}
...
do_help() {
    ...
    echo "     curl -O https://raw.githubusercontent.com/billionairestechnologies/btalgo/main/install/docker-run.sh"
