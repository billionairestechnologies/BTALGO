...
#
# Arrow uses short, single/short codes that differ from BTAlgo's vocabulary:
#   side    : BUY/SELL          -> B / S
...
# --- Order / price type -------------------------------------------------
# BTAlgo pricetype -> Arrow `order` value.
_ORDER_TYPE_MAP = {
    "MARKET": "MKT",
    "LIMIT": "LMT",
    "SL": "SL-LMT",
...
# --- Product type -------------------------------------------------------
# BTAlgo product -> Arrow `product` value.
...
# Arrow `product` -> BTAlgo product (used when normalizing broker responses).
...
def map_order_type(pricetype):
    """Map BTAlgo pricetype to Arrow order type. Defaults to MKT."""
    return _ORDER_TYPE_MAP.get(pricetype, "MKT")


def map_product_type(product):
    """Map BTAlgo product to Arrow product code. Defaults to I (intraday)."""
    return _PRODUCT_MAP.get(product, "I")


def reverse_map_product_type(exchange, product):
    """Map Arrow product code back to BTAlgo product."""
    return _REVERSE_PRODUCT_MAP.get(product)


def map_transaction_type(action):
    """Map BTAlgo BUY/SELL to Arrow B/S."""
    return "B" if str(action).upper() == "BUY" else "S"


def transform_data(data):
    """Transform an BTAlgo order dict into the Arrow place-order payload.

    BTAlgo order fields: symbol, exchange, action, pricetype, quantity,
    ...
    transformed = {
        ...
        "remarks": str(data.get("strategy", "btalgo"))[:16],
    }
...
def transform_modify_order_data(data):
    """Transform an BTAlgo modify-order dict into the Arrow modify payload
