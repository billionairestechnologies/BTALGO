...
def get_client_id(api_key=None):
    ...
    Args:
        api_key: BTAlgo API key (optional)
...
def calculate_single_margin(position_data, auth, client_id):
    ...
    try:
        ...
        logger.debug("STANDARDIZED BTALGO RESPONSE")
        logger.debug("=" * 80)
        logger.debug(f"Standardized Response: {json.dumps(standardized_response, indent=2)}")
        logger.debug("=" * 80)
...
def calculate_margin_api(positions, auth, api_key=None):
    ...
    This limitation is due to Dhan API design, not BTAlgo.

    Args:
        positions: List of positions in BTAlgo format
        auth: Authentication token for Dhan
        api_key: BTAlgo API key (optional, for client ID lookup)
