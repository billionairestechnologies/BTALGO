...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    # Map exchange from BTAlgo format to Motilal format for comparison
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response.get("status") == "SUCCESS":
        ...
        for position in positions_response["data"]:
            ...
            # Convert Motilal exchange to BTAlgo exchange for symbol lookup
            motilal_exchange = position["exchange"]
            btalgo_exchange = reverse_map_exchange(motilal_exchange)

            # Get btalgo symbol to send to placeorder function
            symbol = get_symbol(position["symboltoken"], btalgo_exchange)
            logger.debug(f"The Symbol is {symbol}")

            if not symbol:
                logger.error(
                    f"Symbol not found for token {position['symboltoken']} and exchange {btalgo_exchange}"
                )
                continue
            ...
            place_order_payload = {
                ...
                "symbol": symbol,
                "action": action,
                "exchange": btalgo_exchange,  # Use BTAlgo exchange format
                "pricetype": "MARKET",
                "product": reverse_map_product_type(position["productname"], btalgo_exchange),
