...
def process_definedge_nse_csv(path):
    ...
    try:
        ...
        # Map DefinedGe NSE columns to BTAlgo schema
        # Assuming DefinedGe uses standard format: Symbol, Token, Name, etc.
        processed_df = pd.DataFrame()

        if "Symbol" in df.columns:
            processed_df["symbol"] = df["Symbol"] + "-EQ"  # BTAlgo format
...
def process_definedge_allmaster_csv(path):
    ...
    try:
        ...
        # Format symbols according to BTAlgo standard
        ...
        # Step 3: Explicit rename map for symbols whose cleaned form differs from BTAlgo standard
