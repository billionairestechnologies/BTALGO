...
class ZerodhaExchangeMapper:
    """Maps exchange codes between Zerodha and BTAlgo formats"""

    # Map BTAlgo exchange codes to Zerodha exchange codes
    ...
    # Map Zerodha exchange codes to BTAlgo exchange codes
    ...
    def to_zerodha_exchange(cls, oa_exchange: str) -> str:
        """
        Convert BTAlgo exchange code to Zerodha exchange code.

        Args:
            oa_exchange: BTAlgo exchange code (e.g., 'NSE', 'NFO')
    ...
    def to_oa_exchange(cls, zerodha_exchange: str) -> str:
        """
        Convert Zerodha exchange code to BTAlgo exchange code.

        Args:
            zerodha_exchange: Zerodha exchange code

        Returns:
            BTAlgo exchange code
        """
...
class ZerodhaCapabilityRegistry:
    ...
    # Map BTAlgo capability flags to Zerodha subscription modes
    CAPABILITY_MAP = {
        "LTP": "ltp",
        "QUOTE": "quote",
        "DEPTH": "full",
    ...
    def get_zerodha_mode(cls, capability: str) -> str:
        ...
        Args:
            capability: BTAlgo capability (LTP, QUOTE, DEPTH)
...
class ZerodhaDataTransformer:
    """Transforms data between Zerodha and BTAlgo formats"""
    ...
    def transform_tick(self, tick_data: dict, symbol: str, exchange: str) -> dict:
        """
        Transform Zerodha tick data to BTAlgo format.
        ...
        Returns:
            Transformed tick data in BTAlgo format
        """
        try:
            if not tick_data:
                return {}
    ...
    def transform_order_update(self, order_data: dict) -> dict:
        """
        Transform Zerodha order update to BTAlgo format.

        Args:
            order_data: Raw order data from Zerodha WebSocket

        Returns:
            Transformed order data in BTAlgo format
        """
        try:
            ...
            # Map Zerodha status to BTAlgo status
            status_map = {
                "OPEN": "open",
                "COMPLETE": "complete",
                "CANCELLED": "cancelled",
    ...
    def transform_position(self, position_data: dict) -> dict:
        """
        Transform Zerodha position data to BTAlgo format.

        Args:
            position_data: Raw position data from Zerodha

        Returns:
            Transformed position data in BTAlgo format
        """
        try:
